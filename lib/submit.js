...
(function () {
  ...
  async function submitToAdvisor(state, pdfBytes) {
    ...
    var path = (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random().toString(16).slice(2)) + ".pdf";

    var upload = await client.storage.from("submissions").upload(path, new Blob([pdfBytes], { type: "application/pdf" }), {
      contentType: "application/pdf",
      upsert: false,
    });
    if (upload.error) throw new Error("Échec de l'envoi du PDF : " + upload.error.message);
    ...
    var insert = await client
      ...
      .select()
      .single();
    if (insert.error) throw new Error("Échec de l'enregistrement de la fiche : " + insert.error.message);

    var fn = await client.functions.invoke("send-advisor-email", {
      body: { submissionId: insert.data.id },
    });
    if (fn.error) throw new Error("Le PDF est enregistré, mais l'envoi automatique a échoué : " + fn.error.message);
